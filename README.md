...
- **编辑器一键最小化 / 还原** —— 截图前把 VS Code / Visual Studio 全部收起来，截完再一键展开；
  托盘菜单或全局快捷键（默认 `Ctrl+Shift+D`）都能触发。
- **无边框置顶弹窗** —— 在鼠标旁出现，失焦自动收起，再按一次快捷键也能收起。
- **自动复制到剪贴板** —— 读到文本的那一刻就写进去，按完快捷键直接 `Esc` + `Ctrl+V`。
- **一键保存为本地 txt** —— 固定目录、自动命名。
- **复制按钮** —— 想再复制一次的时候用。
- **自绘启动提示** —— 启动后右下角弹一个仿 Windows 通知的小窗，几秒后自动消失；
  没有用系统通知 API（未打包的应用发通知会被归到别的程序名下），所以图标和应用名始终是自己的。
- **设置窗口** —— 两个快捷键都用「按键录制」方式改，启动提示可开关，保存目录可以浏览选择。
...
6. 想截个图？托盘右键 →「最小化/还原编辑器」，编辑器全收起来，截完再点一次还原
...
| **设置…** | 打开设置窗口（两个快捷键 / 启动提示 / 保存目录） |
| 最小化/还原编辑器（截图前清场） | 把 VS Code / Visual Studio 全部最小化；再点一次还原到原位 |
...
**快捷键（两项都可录制）** —— 点一下对应的框，直接按你想用的组合键，界面实时显示。
`Esc` 取消录制；第二项按 `Backspace` 清空 = 不注册（只用托盘菜单触发）。
至少要有一个修饰键；只按修饰键时会显示 `Ctrl+Shift+…` 等你按主键。支持 `A`–`Z`、`0`–`9`、
`F1`–`F12`、`Space`、`Enter`、`Tab` 以及常见符号键；小键盘和媒体键暂不支持。

| 项 | 默认值 | 作用 |
|---|---|---|
| 读取选中文本 | `Ctrl+Shift+A` | 弹出划词窗口 |
| 最小化/还原编辑器 | `Ctrl+Shift+D` | 截图前把 VS Code / Visual Studio 收起来，再按一次还原 |

> 两条**不能设成同一个键**，设置窗口会直接拦下并提示。
> ⚠️ `Ctrl+Shift+D` 是 VS Code「运行和调试」的默认键 —— 本程序注册的是全局热键，
> 设了它之后 VS Code 里这个快捷键就失效了。介意的话换成别的（比如 `Ctrl+Alt+M`）。

**启动提示** —— 勾选后每次启动，右下角会弹一个仿 Windows 通知的小窗
（应用图标 + 应用名 + 快捷键提示），5 秒后自动消失。不想要就取消勾选。
...
| 字段 | 说明 |
|---|---|
| `hotkey` | 读取选中文本的全局快捷键。写法：`Ctrl+Shift+A`、`Alt+Q`、`Ctrl+Alt+F12`、`Ctrl+Shift+Space` |
| `minimize_hotkey` | 最小化/还原编辑器的全局快捷键。留空或 `null` = 不注册，只用托盘菜单 |
| `save_dir` | 保存目录。`null` 或留空 = 用「文档\text-snap」 |
| `show_startup_toast` | 启动时是否弹自绘通知，`true` / `false` |

> 两条快捷键不能相同 —— 设置窗口会拦下；手改文件的话，后注册的那条会失败并整体回滚。
...
├─ capabilities/default.json     三个窗口的权限集
├─ icons/                        应用与托盘图标（ico + png）
├─ src/
│  ├─ main.rs                    托盘 / 快捷键 / 弹窗 / 命令
│  ├─ config.rs                  配置读写
│  ├─ minimize.rs                编辑器窗口最小化 / 还原
│  ├─ toast.rs                   自绘启动通知窗口
...
- **三个窗口，三种性格。** `main` 是划词弹窗：无边框、置顶、不出现在任务栏、失焦即隐藏、
  启动时就预先创建。`settings` 是常规窗口：有边框、可调大小、按需创建、关掉即销毁。
  `toast` 是启动提示：无边框、置顶、不进任务栏、**不抢焦点**、常驻隐藏按需显示。

- **第三个窗口：toast（启动提示）。** 也是无边框、置顶、不进任务栏，但多了两条：
  **`focusable(false)` 不抢焦点**（截图时弹它不能把焦点从浏览器拿走），以及**常驻隐藏**
  零冷启动。内容是自绘的仿 Windows 通知卡片，前端滑入 / 淡出，后端 5 秒硬隐藏兜底。

- **启动提示不用系统通知 API。** Windows 要求应用有带 AppUserModelID 的开始菜单快捷方式
  才能发 WinRT 通知；未打包的应用发出去会被归到别的程序名下（实测显示成 Windows PowerShell）。
  自己画一个就完全可控了。
...
| 日志里「N/N 个没成功：目标程序可能以管理员运行」 | 编辑器是管理员启动的，UIPI 拦下了最小化操作。text-snap 也以管理员运行即可 |
| 启动后右下角没弹提示 | 设置里的「启动提示」可能被关了；或通知窗口创建失败（日志里有记录） |
| 设置里两条快捷键保存时报「不能使用同一个快捷键」 | 两条设重了，换一个 |
...
- **管理员运行的编辑器也控制不了「最小化/还原」。** 同一个 UIPI 限制：`ShowWindow` 对
  权限更高的窗口会被**静默拦下** —— 不报错、没反应。日志会写
  「N/N 个没成功：目标程序可能以管理员运行」。解决：text-snap 也以管理员身份运行。
...
设置窗口、单实例保护。**后续补充**：编辑器一键最小化/还原、可配置的第二快捷键、
自绘启动提示。
